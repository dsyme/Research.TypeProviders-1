...
\paragraph{Open world and annotated \ident{any} types.}
...
call it an \emph{any} type) annotated with the statically known possibilities (heading, paragraph, image). 
It is ultimamtely mapped to the following F\# type:
%
\begin{equation*}
\begin{array}{l}
...
\end{equation*}
%
This F\# type is worth looking at closely to understand the formalism that follows.  The type
provides statically-typed access to the possible elements known statically from the sample.
However the type is not closed -- the data may be some other element about which nothing is known.
This is the \emph{open-world assumption} -- the user also needs to handle the case when the value is none 
of the statically known elements.  The F\# type simply records the possibility that the value might be one of
the given elements.
%The type is also not exclusive: it doesn't promise that only one of the cases returns a non-\ident{None} value.  

As a consequence, each \ident{any} type can be seen as a top type with annotations. The above code will work
...
contain values relevant to the application. The sample need only specify a selective
...
and a variant is not used if there is an alternative (Corollary~\ref{thm:no-unions}).
