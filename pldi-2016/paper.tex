...
\item The inference mechanism prefers class types with properties. This is to allow extensible (open-world)
  data formats, as we will explain. It also interacts well with developer tooling --
  most editors provide code completion hints on ``.'' and so class types with properties
  are easier to use than types such as discriminated unions that require pattern matching.
...
\paragraph{The role of optional properties.}
...
are available for a person. To support this scenario, our inference algorithm prefers types with properties
and only infers discriminated union types that require pattern matching as the last resort.

In the above example, this is demonstrated by the fact that \ident{Age} is marked as optional.
An alternative is to provide a discriminated union for two different record types (one with \ident{Name} and other with 
...
This is the \emph{open-world assumption}. Supporting the open-world assumption is useful when reading data from 
...
In this case, the possibilities are ultimately presented as the following F\# class type:
%
\begin{equation*}
\begin{array}{l}
...
However this  access is deliberately `weak', because the user needs to explicitly handle 
the case when a value is not of a statically known element. 
...
the programmer is not forced to consider each of the possiblities in the annotations
such as \ident{Paragraph} and \ident{Image}.  This is  consistent with our design principles, 
which prefer optional properties over discriminated unions. The gain is that that the provided types support 
both open-world data and dot-driven developer tooling.
...
Variant-annotated top types can be seen as a family of top types and implicitly permit the \kvd{null} value. 
...
(Theorem~\ref{thm:lub}). When possible, it avoids variant-annotated top types and prefers records 
...
As discussed earlier, variant-annotated top types are generated as F\# classes with properties. Given a variant
